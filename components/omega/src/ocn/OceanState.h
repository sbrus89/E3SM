...
class OceanState {
 ...
 public:
   ...
   I4 getLayerThickness(Array2DReal &LayerThick, int TimeLevel) const;

   /// Get layer thickness host array at given time level
   I4 getLayerThicknessH(HostArray2DReal &LayerThickH, int TimeLevel) const;

   /// Get normal velocity device array at given time level
   I4 getNormalVelocity(Array2DReal &NormVel, int TimeLevel) const;

   /// Get normal velocity host array at given time level
   I4 getNormalVelocityH(HostArray2DReal &NormVelH, int TimeLevel) const;

   /// Exchange halo
   I4 exchangeHalo(int TimeLevel);

   /// Swap time levels to update state arrays
   I4 updateTimeLevels();

   /// Copy state variables from host to device
   I4 copyToDevice(int TimeLevel);

   /// Copy state variables from device to host
   I4 copyToHost(int TimeLevel);
